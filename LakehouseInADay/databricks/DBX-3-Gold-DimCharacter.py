...
dbutils.widgets.text("tableName","DimCharacter","Lake Table")
...
df = (
  df
  .select(col("Class"),col("Race"),col("Name").alias("Player"))
  .distinct()
)
...
(targetDelta.alias('target')
 .merge(df.alias('source'),
        'target.Class = source.Class AND target.Race = source.Race AND target.Player = source.Player')
 .whenNotMatchedInsert(values={"Class":"source.Class","Race":"source.Race","Player":"source.Player"})
 .execute())
...
df = (
  df
  .select(col("Dice"))
  .withColumn("DiceName",concat(lit("D"),col("Dice")))
  .distinct()
)
...
if not DeltaTable.isDeltaTable(spark,f"{targetPath}{tableName.lower()}"):
  ...
  spark.sql(f"""
    ...
    (
      DiceID bigint GENERATED ALWAYS AS IDENTITY,
      Dice string,
      DiceName string
    )
    USING DELTA
...
(targetDelta.alias('target')
 .merge(df.alias('source'),
        'target.Dice = source.Dice')
 .whenNotMatchedInsert(values={"Dice":"source.Dice","DiceName":"source.DiceName"})
 .execute())